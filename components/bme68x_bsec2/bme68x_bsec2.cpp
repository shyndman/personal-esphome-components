...
#define BME68X_BSEC2_ALGORITHM_OUTPUT_LOG(a)                                   \
  (a == ALGORITHM_OUTPUT_CLASSIFICATION ? "Classification" : "Regression")
#define BME68X_BSEC2_OPERATING_AGE_LOG(o)                                      \
  (o == OPERATING_AGE_4D ? "4 days" : "28 days")
...
static const std::string IAQ_ACCURACY_STATES[4] = {"Stabilizing", "Uncertain",
...
void BME68xBSEC2Component::setup() {
  ...
  if (this->bsec2_configuration_ != nullptr &&
      this->bsec2_configuration_length_) {
    this->set_config_(this->bsec2_configuration_,
                      this->bsec2_configuration_length_);
    if (this->bsec_status_ != BSEC_OK) {
      this->mark_failed();
      ESP_LOGE(TAG, "bsec_set_configuration_m failed: status %d",
               this->bsec_status_);
      return;
    }
  }
  ...
  if (this->bsec_status_ != BSEC_OK) {
    this->mark_failed();
    ESP_LOGE(TAG, "bsec_update_subscription_m failed: status %d",
             this->bsec_status_);
    return;
  }
...
void BME68xBSEC2Component::dump_config() {
  ESP_LOGCONFIG(TAG,
                ...
                "    Configured: %s",
                this->version_.major, this->version_.minor,
  ...
  if (this->is_failed()) {
    ESP_LOGE(TAG, "Communication failed (BSEC2 status: %d, BME68X status: %d)",
             this->bsec_status_, this->bme68x_status_);
  ...
  if (this->algorithm_output_ != ALGORITHM_OUTPUT_IAQ) {
    ESP_LOGCONFIG(TAG, "  Algorithm output: %s",
                  BME68X_BSEC2_ALGORITHM_OUTPUT_LOG(this->algorithm_output_));
  }
  ESP_LOGCONFIG(TAG,
                "  Operating age: %s\n"
                "  Sample rate: %s\n"
                ...
                BME68X_BSEC2_OPERATING_AGE_LOG(this->operating_age_),
                BME68X_BSEC2_SAMPLE_RATE_LOG(this->sample_rate_),
                BME68X_BSEC2_VOLTAGE_LOG(this->voltage_),
                this->state_save_interval_ms_, this->temperature_offset_);

#ifdef USE_SENSOR
  LOG_SENSOR("  ", "Temperature", this->temperature_sensor_);
  ESP_LOGCONFIG(TAG, "    Sample rate: %s",
                BME68X_BSEC2_SAMPLE_RATE_LOG(this->temperature_sample_rate_));
  LOG_SENSOR("  ", "Pressure", this->pressure_sensor_);
  ESP_LOGCONFIG(TAG, "    Sample rate: %s",
                BME68X_BSEC2_SAMPLE_RATE_LOG(this->pressure_sample_rate_));
  LOG_SENSOR("  ", "Humidity", this->humidity_sensor_);
  ESP_LOGCONFIG(TAG, "    Sample rate: %s",
                BME68X_BSEC2_SAMPLE_RATE_LOG(this->humidity_sample_rate_));
  LOG_SENSOR("  ", "Gas resistance", this->gas_resistance_sensor_);
  LOG_SENSOR("  ", "CO2 equivalent", this->co2_equivalent_sensor_);
  LOG_SENSOR("  ", "Breath VOC equivalent",
             this->breath_voc_equivalent_sensor_);
...
float BME68xBSEC2Component::get_setup_priority() const {
...
void BME68xBSEC2Component::loop() {
  ...
  // Process a single action from the queue. These are primarily sensor state
  // publishes that in totality take too long to send in a single call.
  if (this->queue_.size()) {
...
void BME68xBSEC2Component::set_config_(const uint8_t *config, uint32_t len) {
  ...
  this->bsec_status_ = bsec_set_configuration_m(
      &this->bsec_instance_, config, len, work_buffer, sizeof(work_buffer));
  if (this->bsec_status_ == BSEC_OK) {
...
float BME68xBSEC2Component::calc_sensor_sample_rate_(SampleRate sample_rate) {
  ...
  return sample_rate == SAMPLE_RATE_ULP ? BSEC_SAMPLE_RATE_ULP
                                        : BSEC_SAMPLE_RATE_LP;
}
...
void BME68xBSEC2Component::update_subscription_() {
  ...
  if (this->iaq_sensor_) {
    ...
    virtual_sensors[num_virtual_sensors].sample_rate =
        this->calc_sensor_sample_rate_(SAMPLE_RATE_DEFAULT);
    num_virtual_sensors++;
  }
  ...
  if (this->pressure_sensor_) {
    ...
    virtual_sensors[num_virtual_sensors].sample_rate =
        this->calc_sensor_sample_rate_(this->pressure_sample_rate_);
    num_virtual_sensors++;
  }
  ...
  if (this->temperature_sensor_) {
    ...
    virtual_sensors[num_virtual_sensors].sample_rate =
        this->calc_sensor_sample_rate_(this->temperature_sample_rate_);
    num_virtual_sensors++;
  }
  ...
  if (this->humidity_sensor_) {
    ...
    virtual_sensors[num_virtual_sensors].sample_rate =
        this->calc_sensor_sample_rate_(this->humidity_sample_rate_);
    num_virtual_sensors++;
  }
#endif
  bsec_sensor_configuration_t sensor_settings[BSEC_MAX_PHYSICAL_SENSOR];
  uint8_t num_sensor_settings = BSEC_MAX_PHYSICAL_SENSOR;
  this->bsec_status_ = bsec_update_subscription_m(
      &this->bsec_instance_, virtual_sensors, num_virtual_sensors,
...
void BME68xBSEC2Component::run_() {
  ...
  this->bsec_status_ = bsec_sensor_control_m(
      &this->bsec_instance_, curr_time_ns, &this->bsec_settings_);
  if (this->bsec_status_ < BSEC_OK) {
    ESP_LOGW(TAG,
             "Failed to fetch sensor control settings (BSEC2 error code %d)",
             this->bsec_status_);
    return;
  }
  ...
  case BME68X_FORCED_MODE:
    ...
    this->bme68x_heatr_conf_.heatr_temp =
        this->bsec_settings_.heater_temperature;
    ...
    // &this->bme68x_);
    status = bme68x_set_heatr_conf(BME68X_FORCED_MODE,
                                   &this->bme68x_heatr_conf_, &this->bme68x_);
  ...
  case BME68X_PARALLEL_MODE:
    if (this->op_mode_ != this->bsec_settings_.op_mode) {
      ...
      this->bme68x_heatr_conf_.enable = BME68X_ENABLE;
      this->bme68x_heatr_conf_.heatr_temp_prof =
          this->bsec_settings_.heater_temperature_profile;
      this->bme68x_heatr_conf_.heatr_dur_prof =
          this->bsec_settings_.heater_duration_profile;
      this->bme68x_heatr_conf_.profile_len =
          this->bsec_settings_.heater_profile_len;
      this->bme68x_heatr_conf_.shared_heatr_dur =
          BSEC_TOTAL_HEAT_DUR -
          (bme68x_get_meas_dur(BME68X_PARALLEL_MODE, &bme68x_conf,
                               &this->bme68x_) /
           INT64_C(1000));

      status = bme68x_set_heatr_conf(BME68X_PARALLEL_MODE,
                                     &this->bme68x_heatr_conf_, &this->bme68x_);
  ...
  if (this->bsec_settings_.trigger_measurement &&
      this->bsec_settings_.op_mode != BME68X_SLEEP_MODE) {
    uint32_t meas_dur = 0;
    meas_dur =
        bme68x_get_meas_dur(this->op_mode_, &bme68x_conf, &this->bme68x_);
...
void BME68xBSEC2Component::read_(int64_t trigger_time_ns) {
  ...
  uint8_t nFields = 0;
  this->bme68x_status_ =
      bme68x_get_data(this->op_mode_, &data[0], &nFields, &this->bme68x_);

  if (this->bme68x_status_ != BME68X_OK) {
    ESP_LOGW(TAG, "Failed to get sensor data (BME68X error code %d)",
             this->bme68x_status_);
    return;
  }
  if (nFields < 1) {
  ...
  for (uint8_t i = 0; i < nFields; i++) {
    bsec_input_t inputs[BSEC_MAX_PHYSICAL_SENSOR]; // Temperature, Pressure,
                                                   // Humidity & Gas Resistance
    uint8_t num_inputs = 0;

    if (BSEC_CHECK_INPUT(this->bsec_settings_.process_data,
                         BSEC_INPUT_TEMPERATURE)) {
      inputs[num_inputs].sensor_id = BSEC_INPUT_TEMPERATURE;
      if (this->temperature_compensation_source_ != nullptr &&
          !std::isnan(this->temperature_compensation_source_->state)) {
        inputs[num_inputs].signal =
            this->temperature_compensation_source_->state;
      } else {
    ...
    if (BSEC_CHECK_INPUT(this->bsec_settings_.process_data,
      ...
      if (this->humidity_compensation_source_ != nullptr &&
          !std::isnan(this->humidity_compensation_source_->state)) {
        inputs[num_inputs].signal = this->humidity_compensation_source_->state;
      } else {
    ...
    if (BSEC_CHECK_INPUT(this->bsec_settings_.process_data,
      ...
      inputs[num_inputs].signal =
          (this->op_mode_ == BME68X_FORCED_MODE) ? 0 : data[i].gas_index;
      inputs[num_inputs].time_stamp = trigger_time_ns;
      num_inputs++;
    }
    ...
    this->bsec_status_ = bsec_do_steps_m(&this->bsec_instance_, inputs,
                                         num_inputs, outputs, &num_outputs);
    if (this->bsec_status_ != BSEC_OK) {
      ESP_LOGW(TAG, "BSEC2 failed to process signals (BSEC2 error code %d)",
               this->bsec_status_);
      return;
    }
    if (num_outputs < 1) {
...
void BME68xBSEC2Component::publish_(const bsec_output_t *outputs,
                                    uint8_t num_outputs) {
...
#ifdef USE_SENSOR
      this->queue_push_([this, signal]() {
        this->publish_sensor_(this->iaq_sensor_, signal);
      });
#endif
      break;
...
#ifdef USE_SENSOR
      this->queue_push_([this, signal]() {
        this->publish_sensor_(this->iaq_static_sensor_, signal);
      });
#endif
      break;
...
#ifdef USE_SENSOR
      this->queue_push_([this, signal]() {
        this->publish_sensor_(this->co2_equivalent_sensor_, signal);
      });
#endif
      break;
...
#ifdef USE_SENSOR
      this->queue_push_([this, signal]() {
        this->publish_sensor_(this->breath_voc_equivalent_sensor_, signal);
      });
#endif
      break;
...
#ifdef USE_SENSOR
      this->queue_push_([this, signal]() {
        this->publish_sensor_(this->pressure_sensor_, signal / 100.0f);
      });
#endif
      break;
...
#ifdef USE_SENSOR
      this->queue_push_([this, signal]() {
        this->publish_sensor_(this->gas_resistance_sensor_, signal);
      });
#endif
      break;
...
#ifdef USE_SENSOR
      this->queue_push_([this, signal]() {
        this->publish_sensor_(this->temperature_sensor_, signal);
      });
#endif
      break;
...
#ifdef USE_SENSOR
      this->queue_push_([this, signal]() {
        this->publish_sensor_(this->humidity_sensor_, signal);
      });
#endif
      break;
    }
  ...
  if (update_accuracy) {
    ...
    this->queue_push_([this, max_accuracy]() {
      this->publish_sensor_(this->iaq_accuracy_sensor_, max_accuracy, true);
    });
#endif
#ifdef USE_TEXT_SENSOR
    this->queue_push_([this, max_accuracy]() {
      this->publish_sensor_(this->iaq_accuracy_text_sensor_,
                            IAQ_ACCURACY_STATES[max_accuracy]);
    });
#endif
    // Queue up an opportunity to save state
    this->queue_push_(
        [this, max_accuracy]() { this->save_state_(max_accuracy); });
  }
}
...
int64_t BME68xBSEC2Component::get_time_ns_() {
  ...
  return (time_ms + ((int64_t)this->millis_overflow_counter_ << 32)) *
         INT64_C(1000000);
}

#ifdef USE_SENSOR
void BME68xBSEC2Component::publish_sensor_(sensor::Sensor *sensor, float value,
                                           bool change_only) {
  if (!sensor ||
      (change_only && sensor->has_state() && sensor->state == value)) {
    return;
  }
...
#ifdef USE_TEXT_SENSOR
void BME68xBSEC2Component::publish_sensor_(text_sensor::TextSensor *sensor,
...
void BME68xBSEC2Component::load_state_() {
  ...
  this->bsec_state_ =
      global_preferences->make_preference<uint8_t[BSEC_MAX_STATE_BLOB_SIZE]>(
          hash, true);
  ...
  if (this->bsec_state_.load(&state)) {
    ...
    this->bsec_status_ =
        bsec_set_state_m(&this->bsec_instance_, state, BSEC_MAX_STATE_BLOB_SIZE,
                         work_buffer, sizeof(work_buffer));
    if (this->bsec_status_ != BSEC_OK) {
      ESP_LOGW(TAG, "Failed to load state (BSEC2 error code %d)",
               this->bsec_status_);
    }
...
void BME68xBSEC2Component::save_state_(uint8_t accuracy) {
  if (accuracy < 3 ||
      (millis() - this->last_state_save_ms_ < this->state_save_interval_ms_)) {
    return;
  }
  ...
  this->bsec_status_ = bsec_get_state_m(
      &this->bsec_instance_, 0, state, BSEC_MAX_STATE_BLOB_SIZE, work_buffer,
      BSEC_MAX_STATE_BLOB_SIZE, &num_serialized_state);
  if (this->bsec_status_ != BSEC_OK) {
    ESP_LOGW(TAG, "Failed fetch state for save (BSEC2 error code %d)",
             this->bsec_status_);
    return;
  }
