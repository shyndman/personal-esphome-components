...
async def to_code_base(config):
    ...
    if compensation := config.get(CONF_COMPENSATION):
        if humidity_source := compensation.get(CONF_HUMIDITY_SOURCE):
            sens = await cg.get_variable(humidity_source)
            cg.add(var.set_humidity_compensation_source(sens))
        if temperature_source := compensation.get(CONF_TEMPERATURE_SOURCE):
            sens = await cg.get_variable(temperature_source)
            cg.add(var.set_temperature_compensation_source(sens))
